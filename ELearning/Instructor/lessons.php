...
if ($resultCourse->num_rows > 0) {
    while ($rowCourse = $resultCourse->fetch_assoc()) {
        ...
        $sql = "SELECT * FROM lesson WHERE course_id = $coursesId";
        ...
        if ($result->num_rows > 0) {  
            ...
            <div class="table-responsive">
              <table class="table align-items-center mb-0">
                <thead>
                  <tr>
                    <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Lesson ID</th>
                    <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Lesson Name</th>
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<td class="text-center">' . $row["lesson_name"] . '</td>';
                ...
                echo '<td class="text-center">
                        ...
                        <button class="btn btn-secondary delete-btn" data-id="' . $row["lesson_id"] . '" data-bs-toggle="modal" data-bs-target="#deleteModal"><i class="far fa-trash-alt"></i></button>
                      </td>';
                echo '</tr>';
            }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.delete-btn').forEach(button => {
      button.addEventListener('click', function () {
          const lessonId = this.getAttribute('data-id'); // Get lesson_id from data-id attribute
          document.getElementById('lessonId').value = lessonId; // Set the hidden input in the modal to lesson_id
